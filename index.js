...
if (!USER || !PASS) {
    console.error(JSON.stringify({ error: "Missing institutional credentials" }));
    process.exit(1);
}
...
async function login() {
    ...
    // silence intermediate logs
    return { cookies: allCookies, dashboardHtml: dashboardRes.data, loginResponse: response };
}

(async () => {
    try {
        const { cookies, dashboardHtml, loginResponse } = await login();
        if (loginResponse.authorised) {
...
async function fetchdetails(cookies, dashboardHtml) {
    ...
    process.stdout.write(JSON.stringify(studentData) + "\n");
    return studentData;
}