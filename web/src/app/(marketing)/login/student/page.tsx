          ...
          >
            ...
            <form onSubmit={handleLogin} className="space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] uppercase tracking-widest opacity-40 font-bold italic">Registration Number</label>
                <input 
                  type="text" 
                  value={uname}
              ...
              <div className="space-y-2">
                <label className="text-[10px] uppercase tracking-widest opacity-40 font-bold italic">VTOP Password</label>
                <input 
                  type="password" 
                  value={passwd}
              ...
              {/* Automated Handshake: Captcha solved at the backend node */}

              {error && <p className="text-error text-[10px] uppercase tracking-widest font-bold text-center mt-4 italic">! {error}</p>}

              <button 
                type="submit"
                disabled={loading}
                className="w-full bg-primary/80 hover:bg-primary text-on-primary font-bold py-5 rounded-2xl tracking-[0.2em] transition-all disabled:opacity-50 uppercase text-xs"
              >
                {loading ? "SOLVING HANDSHAKE..." : "INITIATE IDENTITY SYNC"}
              </button>
            </form>

            <p className="mt-12 text-[9px] uppercase tracking-[0.3em] opacity-30 text-center leading-loose italic">
              Secure Handshake Enabled.<br />
              Institutional biometric data is zero-trace.
            </p>
          </motion.div>
        ) : (
          <motion.div 
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-4 pt-4 md:pt-0">
                    <h4 className="text-[9px] uppercase tracking-widest opacity-30 border-l border-primary pl-2 italic">Kinship & Profile</h4>
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <span className="text-[8px] uppercase opacity-40 block mb-1">Blood Group</span>
                            <span className="text-[10px] font-bold text-primary italic">{session.blood_group}</span>
                        </div>
                        <div>
                            <span className="text-[8px] uppercase opacity-40 block mb-1">DOB Node</span>
            ...
            <div className="mt-10 flex space-x-4">
                <button 
                    ...
                    className="flex-1 bg-primary/80 text-on-primary font-bold py-4 rounded-2xl flex items-center justify-center space-x-3 hover:bg-primary transition-all uppercase tracking-widest text-[10px]"
                >
                    <span>Enter Sanctuary Hub</span>
                    <ArrowRight className="w-4 h-4" />
                </button>
                <button 
                    onClick={() => setSession(null)}
                    className="px-6 py-4 border border-white/5 rounded-2xl opacity-40 hover:opacity-100 transition-all text-[9px] uppercase tracking-widest font-bold italic"
                >
                    Switch
                </button>
            </div>
